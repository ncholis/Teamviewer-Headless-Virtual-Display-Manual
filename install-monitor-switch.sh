...
cat > /usr/local/bin/auto-switch-display.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x /usr/local/bin/auto-switch-display.sh

cat > /etc/systemd/system/teamviewer-display-monitor.service <<EOF
[Unit]
Description=Auto switch TeamViewer display between physical and dummy monitor
After=graphical.target teamviewerd.service gdm.service network.target
Wants=graphical.target teamviewerd.service

[Service]
Type=simple
ExecStart=/usr/local/bin/auto-switch-display.sh ${USERNAME}
Restart=always
RestartSec=5
User=root
